...
\filbreak
\subsubsection{SPFA}\index{spfa}
\nopagebreak{\footnotesize\sffamily\color{blurb} SPFA (Bellman-Ford with a queue) - shortest paths with NEGATIVE edges, plus negative-cycle\par}\nopagebreak
\begin{lstlisting}
...
\filbreak
\subsubsection{ThreeEdgeConnectivity}\index{threeEdgeCC}
\nopagebreak{\footnotesize\sffamily\color{blurb} THREE-EDGE-CONNECTED COMPONENTS in ONE DFS, O(n + m). Undirected, 0-indexed, may be\par}\nopagebreak
\begin{lstlisting}
...
\filbreak
\subsubsection{OnlineBridges}\index{OnlineBridges}
\nopagebreak{\footnotesize\sffamily\color{blurb} ONLINE (INCREMENTAL) BRIDGES - maintain the number of bridges while edges are ADDED, in\par}\nopagebreak
\begin{lstlisting}
...
\filbreak
\subsubsection{FloorSumExtended}\index{FGH}\index{fgh}
\nopagebreak{\footnotesize\sffamily\color{blurb} EUCLIDEAN-LIKE ALGORITHM, THE FULL TRIPLE. With q\_i = floor((a*i + b) / c) it returns\par}\nopagebreak
\begin{lstlisting}
...
\filbreak
\subsubsection{PrimeEnumeration1e9}\index{sievePrimes}
\nopagebreak{\footnotesize\sffamily\color{blurb} ENUMERATE EVERY PRIME UP TO 1e9 IN \textasciitilde{}0.5s, and in a 30-smooth wheel that costs 1 BIT per 30\par}\nopagebreak
\begin{lstlisting}
